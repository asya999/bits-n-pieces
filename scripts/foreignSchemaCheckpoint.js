...
ifempty = function(f, n) {
    ...
    ifc["$cond"]["if"]={};
    ifc["$cond"]["if"]["$eq"]=[];
    ifc["$cond"]["if"]["$eq"].push(f);
    ifc["$cond"]["if"]["$eq"].push("");
    ifc["$cond"]["then"]=f;
    ifc["$cond"]["else"]=n;  /* maybe should be {"$literal":n} */
    return ifc;
}
...
prepSchema = function(mschema, dbname, coll, tablename, result) {
    ...
    else if (!topLevel) prefix=tablename.slice(tablename.indexOf("__")+2);
         else throw "tablename is passed in, but it's top level!";
    ...
    for (field in mschema) {
        ...
        if ( currentfield.hasOwnProperty("#array") && currentfield["#array"] ) {
           ...
           if (currentfield["#type"]=="object" || currentfield["#type"].hasOwnProperty("object") ) {
              for (g in fschema) if (currentfield.hasOwnProperty(g)) fschema[field][g]["#proj"]="$"+field+"."+g; 
    ...
    for (f in sch) {
        ...
        if ( !topLevel && f!=prefix ) {
           prf="$"+prefix+"."+prf;
        } 
        if (sch[f]["#pgtype"]=="numeric") {
           prf=ifempty(ifnull(prf));
        } 
